...
reverse_timer = int(input("Введите время до обнуления таймера: "))

for i in range(reverse_timer, 0, -1):
    answer = int(input(f"Осталось {i} секунд. Готовы забрать еду? Введите ‘1’ - Да, ‘0’ - Нет: "))

    if answer == 1:
        print(f"Ваша еда готова, можете забрать. Прервано на {i} секунде.")
        break
    elif answer == 0:
        if i == 1:
